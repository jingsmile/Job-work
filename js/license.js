  ...
  //判断添加图片有没有成功;
      if(reader==!function(evt){img.src = evt.target.result;}){
            ...
            $('.btn').click(function(){
                ...
                $('.limit').hide();
                $('.foottxt').hide();
                $('.alert').html("您的营业执照正在审核,请耐心等待!").addClass('alert_success');
...
function uploadImage(){
      var imgPath=$('#previewImg').val();
      if(imgPath==""){
          $('.alert').html("请您先添加图片！");
          return;
      };
      var strSuffix=imgPath.substring(imgPath.lastIndexOf('.')+1);
      if(strSuffix!='jpg'&&strSuffix!='gif'&&strSuffix!='png'){
          $('.alert').html("您上传的图片不符合,请重新上传！");
          return;
      };
      $.ajax({
          type:'post',
          url:'file:///C:/wamp/www/D_zhilian/frame2.html',
          data:{
              imgPath: $("#previewImg").val()
          },
          ...
          error:function(XMLHttpRequest, textStatus, errorThrown){
              $('.alert').html("您上传的营业执照审核未通过,请您重新上传");
              $('<i></i>').appendTo(oWarn).addClass('light');
              $('.btn').show().val("重新上传");
          }
      })
};*/